...
import uk.gov.hmrc.vapingstampsapi.models.errors.{BusinessError, MultipleStampsRequestReceived, StampsReferenceNumberNotFound, VdsEmailNotFound}
  ...
  "BusinessError" must {
    "toString" must {
      ...
      "StampRequestedBefore must be 003" in {
        MultipleStampsRequestReceived.toString mustBe "003"
      }
    }
    ...
    "write as JsValue" when {
      ...
      "write StampRequestedBefore to JsString(003)" in {
        Json.toJson(MultipleStampsRequestReceived)(using BusinessError.format) mustBe JsString("003")
      }
